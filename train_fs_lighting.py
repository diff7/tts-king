...
class LTraier(pl.LightningModule):
    def __init__(self,cfg, model, loss_fn, scheduled_optim, optimizer, voocoder):
        super().init__()
    ...
    def one_step(self, batches, validation=True):
        ...
        for i, batch in enumerate(batches):
                ...
                for j, data_of_batch in enumerate(batch):
                    ...
                    btach_items_keys = ["text","D","log_D","f0","energy","src_len","mel_len"]
                    ...
                    for l_name, value in zip(losses_names, [total_loss] + losses_values_items):
        ...
        i+=1
        j+=1
        losses_values = [losses_values[l_name]/((i*j)) for l_name in losses_names]
    ...
    def validation_step(self, batches, batch_nb):
            ...
            return {
            "val_loss": total_losses,
            "val_log": losses_values}}
    ...
    def synth_step(self, 
        ...
        if vocoder is not None:
            ...
            self.logger.experiment.log({"examples_audio": [wandb.Audio(speech_gen, caption="Generated", sample_rate=16)]})
            self.logger.experiment.log({"examples_audio": [wandb.Audio(speech_true, caption="Ground truth", sample_rate=16)]})

        self.logger.experiment.log({"examples_mel": [wandb.Image(mel_target, caption="Mel Target")]})
        self.logger.experiment.log({"examples_mel": [wandb.Image(speech_true, caption="Mel postnet output")]})
...
def train_fs(cfg,
        ...
        if resume_lighting is not None:
            resume_from = os.path.join(weights_dir, resume_lighting)
            print(resume_from)

        trainer = pl.Trainer( resume_from_checkpoint = resume_from,
