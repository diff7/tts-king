...
from fs_two.utils import get_alignment
from fs_two.text import _clean_text
...
def prepare_align(in_dir):
    with open(os.path.join(in_dir, "metadata.csv"), encoding="utf-8") as f:
        for line in f:
            ...
            with open(
                os.path.join(in_dir, "wavs", "{}.txt".format(basename)), "w"
            ) as f1:
                f1.write(text)
...
def build_from_path(in_dir, out_dir):
    ...
    return [r for r in train if r is not None], [
        r for r in val if r is not None
    ]
...
def process_utterance(in_dir, out_dir, basename):
    ...
    _, wav = read(wav_path)
    wav = wav[
        int(hp.sampling_rate * start) : int(hp.sampling_rate * end)
    ].astype(np.float32)
    ...
    np.save(
        os.path.join(out_dir, "alignment", ali_filename),
        duration,
        allow_pickle=False,
    )
    ...
    np.save(
        os.path.join(out_dir, "energy", energy_filename),
        energy,
        allow_pickle=False,
    )
